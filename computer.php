...
if ($argc > 1)
{
	...
	if ($check[0] || preg_match_all("/[X]\^-\d/", $argv[1], $check2))
	{
		echo "Syntax error\n";
		exit();
	}
...
function 		transfert($str, $str2, $deg)
{
	$str = trim($str);
	$reg = "/([\+\-]?)\s?((\d+[.]?\d?+)\s?[*]\s?[X]\^" . $deg . ")/";
